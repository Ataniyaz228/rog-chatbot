...
export default function Home() {
  ...
  // Stable pending conversation ID (only generated once until used)
  ...
  const handleNewChat = useCallback(() => {
    ...
    setMessages([]);
    setDocuments([]);
    pendingIdRef.current = null; // Reset so a new ID is generated for the next new chat
  }, []);
  ...
  const handleSend = useCallback(async (message: string) => {
    ...
    try {
      ...
      // Update active ID if this is a new conversation
      if (!activeId) {
        setActiveId(response.conversationId);
        pendingIdRef.current = null; // Used up the pending ID
      }
  ...
  const handleDocumentUploaded = useCallback((doc: DocumentInfo) => {
    ...
    // Set active ID if first upload in new chat
    if (!activeId) {
  ...
  return (
    <div className="flex h-screen w-screen bg-transparent p-4 gap-4 overflow-hidden relative z-10 box-border">
      <Sidebar
        ...
        onToggle={useCallback(() => setSidebarCollapsed(p => !p), [])}
